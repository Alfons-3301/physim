...
def simulate_bsc_with_polar(p_flip,µ,hash, input_bits,codec):
    ...
    # Calculate BER
    ber = np.sum(input_bits != decoded_bits) / len(input_bits) + 1e-15
...
if __name__ == "__main__":
    ...
    hash = PrivacyAmplification2(µ,K_polar)
    ...
    ber_bob,bler_bob = simulate_bsc_with_polar(0.1, µ, hash, input_bits, codec_bob)
    ber_eve,bler_eve = simulate_bsc_with_polar(0.2, µ, hash, input_bits, codec_eve)
